...
class WorkerPolicyTests(unittest.TestCase):
    ...
    def test_public_catalog_overrides_are_available_without_secrets(self) -> None:
        ...
        self.assertIn("authorBioForName", override_source)
    ...
    def test_access_worker_requires_secret_and_private_map(self) -> None:
        ...
        self.assertIn("metadata/reader-comments/", source)
        self.assertIn("handleReaderComments", source)
        self.assertIn("env.BOOK_FILES.put", source)
        self.assertNotIn(".delete(", source)
        self.assertNotIn(".list(", source)

    def test_reader_comments_are_page_scoped_and_admin_deletable(self) -> None:
        access_source = ACCESS_WORKER.read_text(encoding="utf-8")
        admin_source = UPLOAD_WORKER.read_text(encoding="utf-8")
        reader_js = (ROOT / "public" / "assets" / "reader.js").read_text(encoding="utf-8")
        admin_js = (ROOT / "public" / "assets" / "admin.js").read_text(encoding="utf-8")

        self.assertIn("/reader-comments/", access_source)
        self.assertIn("readerCommentsKey(bookId, pageNumber)", access_source)
        self.assertIn("validateReaderToken(bookId, token, env)", access_source)
        self.assertIn("cleanMultilineText", access_source)
        self.assertIn("/admin/comments", admin_source)
        self.assertIn("deleteReaderComment", admin_source)
        self.assertIn("Access-Control-Allow-Methods", admin_source)
        self.assertIn("DELETE", admin_source)
        self.assertIn("data-reader-comments", reader_js)
        self.assertIn("readerCommentsUrl", reader_js)
        self.assertIn("查看/发表本页留言", reader_js)
    ...
    def test_reader_page_lazy_loads_reader_images(self) -> None:
        ...
        self.assertIn("data-reader-floating-actions", reader_page)
        self.assertIn("data-reader-scroll-current", reader_page)
        self.assertIn("data-reader-floating-detail", reader_page)
        self.assertIn("--reader-zoom", reader_js)
        self.assertIn("attachReaderZoomGestures", reader_js)
        self.assertIn("attachReaderPanAndPinchGestures", reader_js)
        self.assertIn("dblclick", reader_js)
        self.assertIn("event.ctrlKey", reader_js)
        self.assertIn("setPointerCapture", reader_js)
        self.assertIn("activeReaderPointers", reader_js)
        self.assertIn("floatingDetailLink.href = detailHref", reader_js)
        self.assertIn("window.scrollTo({ top: 0, behavior: \"smooth\" })", reader_js)
