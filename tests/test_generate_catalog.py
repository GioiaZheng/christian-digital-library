...
class CatalogGenerationTests(unittest.TestCase):
    ...
    def test_sample_book_generates_public_catalog_and_detail_page(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn("读者看法", detail)
            self.assertIn('data-book-opinions', detail)
            self.assertIn('data-book-opinions-form', detail)
    ...
    def test_multiple_author_bios_are_split_by_author_heading(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            project = create_sample_project(Path(directory))
            rows = [
                {
                    "id": "multi-author-bio-book",
                    "clean_title": "Multi Author Bio Book",
                    "author": "Alpha Author, Beta Author",
                    "author_bio": "Alpha Author: Alpha bio.\nBeta Author: Beta bio.",
                    "translator": "",
                    "publisher": "",
                    "year": "2024",
                    "language": "中文",
            ...
            alpha_page = output / "authors" / f"{GENERATOR.author_slug('Alpha Author')}.html"
            beta_page = output / "authors" / f"{GENERATOR.author_slug('Beta Author')}.html"
            alpha_html = alpha_page.read_text(encoding="utf-8")
            beta_html = beta_page.read_text(encoding="utf-8")

            self.assertIn("<p data-author-profile-bio>Alpha bio.</p>", alpha_html)
            self.assertIn("<p data-author-profile-bio>Beta bio.</p>", beta_html)
    ...
    def test_admin_page_is_generated_with_separate_login(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertIn('id="admin-comments-summary"', admin)
            self.assertIn('id="admin-comments-list"', admin)
            self.assertIn("阅读评论管理", admin)
            self.assertIn('class="admin-sidebar"', admin)
            self.assertIn('data-admin-target="overview"', admin)
            self.assertIn('data-admin-section="add"', admin)
            self.assertIn('data-admin-section="uploads"', admin)
            self.assertIn('data-admin-section="edit"', admin)
